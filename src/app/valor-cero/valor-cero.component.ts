...
@Component({
  selector: 'app-valor-cero',
  standalone: true,
  imports: [NgxDatatableModule, RouterOutlet, CommonModule, NgIf, FormsModule,ReactiveFormsModule, RouterModule, MatExpansionModule],
  templateUrl: './valor-cero.component.html',
  styleUrl: './valor-cero.component.css',
  providers: [provideNativeDateAdapter(), AdminService, AssetService, { provide: LOCALE_ID, useValue: 'es-MX' }, MatDialogModule,],
})
export class ValorCeroComponent {
  ...
  openDialogPreView(id: string): void {
    const assetToEdit = this.assets.find(asset => asset._id === id);
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && assetToEdit) {
          // Assuming result contains the updated asset
          this.updateAsset(assetToEdit._id, result);
      } else {
          console.error('No asset returned from dialog or assetToEdit is undefined.');
      }
  });
  }
  resetFilters(): void {
  ...
  getAssets() {
    this._route.params.forEach((params: Params) => {
      ...
      this._assetService.getAssetsCero(this.token, page).subscribe(
        (response) => {
          if (!response.assets) {
  ...
  updateAsset(assetId: string, updatedAsset: AssetLIST) {
    ...
    if (index !== -1) {
      ...
      this.rows = [...this.assets]; // Refresh the displayed rows
    }
  }
  public onSubmit() {
